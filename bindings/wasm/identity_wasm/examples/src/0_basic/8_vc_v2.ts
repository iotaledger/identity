...
export async function vcV2() {
    ...
    const credentialJwt = await issuerDocument.createCredentialV2Jwt(
        issuerStorage,
        issuerFragment,
        unsignedVc,
    ...
    const res = new JwtCredentialValidator(new EdDSAJwsVerifier()).validateV2(
        credentialJwt,
        issuerDocument,
    ...
    const unsignedVp = Presentation.newV2({
        holder: aliceDocument.id(),
        verifiableCredential: credentialJwt.intoEnvelopedVc(),
    });
    ...
    let jwtCredentials: JwtVcV2[] = decodedPresentation
        .presentation()
        .verifiableCredential()
        .map((credential) => {
            const envelopedVc = credential.tryIntoEnvelopedVc();
            if (!envelopedVc) {
                throw new Error("expected an enveloped verifiable credential");
            } else {
                return JwtVcV2.fromEnvelopedVc(envelopedVc);
            }
        });
    ...
    for (let i = 0; i < jwtCredentials.length; i++) {
        credentialValidator.validateV2(
            jwtCredentials[i],
            resolvedIssuers[i],
            validationOptions,
            FailFast.FirstError,
