...
async fn main() -> anyhow::Result<()> {
  ...
  let credential: CredentialV2 = CredentialBuilder::default()
    ...
    .subject(subject)
    .build_v2()?; // Note: we are building a VC Data Model 2.0 credential!

  let credential_jwt: JwtVcV2 = issuer_document
  ...
  let presentation: Presentation<EnvelopedVc> =
    PresentationBuilder::new(holder_document.id().to_url().into(), Default::default())
      // Note: the JWT string is appropriately enveloped as mandated by the JOSE specification.
      .credential(credential_jwt.into_enveloped_vc())
      .build_v2()?; // Note: we are building a VC Data Model 2.0 presentation!
  ...
  let presentation: DecodedJwtPresentation<EnvelopedVc> = JwtPresentationValidator::with_signature_verifier(
  ...
  let jwt_credentials: Vec<JwtVcV2> = presentation
    .presentation
    .verifiable_credential
    .clone()
    .into_iter()
    .map(|enveloped_vc| JwtVcV2::try_from(enveloped_vc).unwrap())
    .collect();
  ...
  for (index, jwt_vc) in jwt_credentials.iter().enumerate() {
    ...
    let _decoded_credential: DecodedJwtCredentialV2<Object> = credential_validator
      .validate_v2::<_, Object>(jwt_vc, issuer_document, &validation_options, FailFast::FirstError)
      .unwrap();
  }
