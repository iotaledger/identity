...
use iota_sdk::move_types::iota_framework::object::ID;
use phf::phf_map;
use phf::Map;
...
pub(crate) async fn migration_registry_id(client: &IdentityClient) -> Result<ObjectId, Error> {
  let network = client.network();
  let network_id = network.as_chain_id();
  ...
  let package_id = identity_package_id(network)
    .await
    .map_err(|_| Error::UnknownNetwork(network))?;
...
pub enum Error {
  ...
  #[error(transparent)]
  Identity(#[from] IdentityResolutionError),
}

/// Lookup a legacy `alias_id` into the migration registry
/// to get the ID of the corresponding migrated DID document, if any.
pub async fn lookup(id_client: &IdentityClient, alias_id: ObjectId) -> Result<Option<OnChainIdentity>, Error> {
  ...
  let Some(df_value) = id_client
    .dynamic_field(registry_id.into(), type_, alias_id)
    .await?
  ...
  Ok(get_identity(id_client, id).await?)
}

async fn find_migration_registry(client: &IdentityClient, package_id: ObjectId) -> Result<ObjectId, Error> {
  ...
  let event_filter =
    EventFilter::default().with_event_type(format!("{package_id}::migration_registry::MigrationRegistryCreated"));
  ...
  let event = events.swap_remove(0);
